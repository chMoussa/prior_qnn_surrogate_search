...
if __name__ == "__main__":
	...
	for t_idx, task_id in enumerate(task_ids):
	    ...
	    for train,test in cv_obj.split(X_data):
	        ...
	        model = fANOVA_surrogate(X=X_data_train, Y=y_data_train, n_trees=n_trees, seed=t_idx)
	        ytesthat = model.predict(X_data_test)
	        result_cv["r2"].append(r2_score(y_data_test,ytesthat))
	        result_cv["rmse"].append(mean_squared_error(y_data_test,ytesthat, squared=False))
	    ...
	    print(task_id, np.mean(result_cv['r2']), np.mean(result_cv['rmse']), np.mean(result_cv['cc']))